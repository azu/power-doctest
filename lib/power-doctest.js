...
function setRegexpExecuted(regexp) {
...
var convertTree = function (ast, options) {
    ...
    if (options.regexpExecuted) {
        setRegexpExecuted(options.regexpExecuted)
    }
    rocambole.recursive(ast, function (node) {
        if (power.isExpression(node)) {
            ...
            if (nextElement && power.isDocComment(nextElement)) {
                var commentExpression = power.getCommentAsExpression(nextElement);
                power.replaceToAssert(node, commentExpression);
            }
        }
    });
    return ast;
